...
class Phi2Config(PretrainedConfig):
    ...
    def load_config(cls, path):
        if path in PHI2_STANDARD_CONFIGS:
            return cls.from_dict(PHI2_STANDARD_CONFIGS[path])
...
def NewGELUActivation(input):
    ...
    """
    return 0.5 * input * (1.0 + jnp.tanh(math.sqrt(2.0 / math.pi) * (input + 0.044715 * jnp.power(input, 3.0))))
...
class FlaxPhi2Attention(nn.Module):
    config: Phi2Config
    dtype: jnp.dtype=jnp.float32
...
class FlaxPhi2MLP(nn.Module):
    ...
    def __call__(self, x: jnp.ndarray, enable_dropout: bool = False) -> jnp.ndarray:
        x = self.w2(NewGELUActivation(self.w1(x)))
        return x


class FlaxPhi2Block(nn.Module):
    config: Phi2Config
    dtype: jnp.dtype=jnp.float32
    param_dtype: jnp.dtype=jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]]=None
...
class FlaxPhi2PreTrainedModel(FlaxPreTrainedModel):
    """
...
class FlaxPhi2BlockCollection(nn.Module):
    config: Phi2Config
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype=jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]]=None
    ...
    def setup(self):
        ...
        if self.config.remat_block != '':
            block = remat(
                FlaxPhi2Block, static_argnums=(3, 4, 5),
...
class FlaxPhi2Module(nn.Module):
    config: Phi2Config
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype=jnp.float32
    precision: Optional[Union[jax.lax.Precision, str]]=None
    ...
    def setup(self):
        ...
        self.h = FlaxPhi2BlockCollection(self.config, dtype=self.dtype, param_dtype=self.param_dtype, precision=self.precision)
...
class FlaxPhi2MAModel(FlaxPhi2PreTrainedModel):
    module_class = FlaxPhi2Module
...
class FlaxPhi2ForCausalLMModule(nn.Module):
    config: Phi2Config
    dtype: jnp.dtype = jnp.float32
    param_dtype: jnp.dtype=jnp.float32
    ...
    def setup(self):
        self.transformer = FlaxPhi2Module(self.config, dtype=self.dtype)
...
@add_start_docstrings("", "")
class FlaxPhi2ForCausalLM(FlaxPhi2PreTrainedModel):
    module_class = FlaxPhi2ForCausalLMModule
