'''
Adapt from https://github.com/young-geng/EasyLM/blob/main/EasyLM/checkpoint.py
'''
...
from module.jax_utils import tree_apply, float_tensor_to_dtype
...
class StreamingCheckpointer(object):
    ...
    def save_checkpoint(self, train_state, filename, gather_fns=None, trainable_params=None):
        if self.enable:
            path = os.path.join(self.checkpoint_dir, filename)
        else:
            path = '/dev/null'
        if self.config.trainable_only:
            to_save = trainable_params
        else:
            to_save = None
        self.save_train_state_to_file(
            train_state, path, gather_fns, self.config.float_dtype, to_save
        )

    @staticmethod
    def save_train_state_to_file(train_state, path, gather_fns=None,
                                 float_dtype=None, to_save=None):
        ...
        with mlxu.open_file(path, "wb") as fout:
            for key, value in flattend_train_state.items():
                if to_save and key not in to_save:
                    continue
                if gather_fns is not None:
    ...
    def save_all(self, train_state, gather_fns, metadata=None,
                 dataset=None, milestone=False, trainable_parameters=None):
    ...
    @staticmethod
    def load_checkpoint(path, target=None, shard_fns=None, remove_dict_prefix=None, random_trainstate=None):
            ...
            for key, value in unpacker:
                ...
                if shard_fns is not None:
                    if key not in shard_fns:
                        new_key = key[0:1] + ('inner_opt_state',) + key[1:]
                        if new_key in shard_fns:
                            key = new_key                            
                    tensor = shard_fns[key](tensor)
                flattend_train_state[key] = tensor
                
        if random_trainstate is not None:
            flat_random_trainstate = flatten_dict(
                to_state_dict(random_trainstate), keep_empty_nodes=True)           
            ...
            for key, value in flattened_target.items():
                if key not in flattend_train_state:
                    if value == empty_node:
                        ...
                        if random_trainstate is not None:
                            flattend_train_state[key] = flat_random_trainstate[key]
    ...
    def load_trainstate_checkpoint(cls, load_from, trainstate_target=None,
        ...
        if load_type == 'trainstate':
            ...
            train_state = cls.load_checkpoint(
                ...
                random_trainstate=random_trainstate,
            )
