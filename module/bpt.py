"""
From https://github.com/young-geng/EasyLM/blob/main/EasyLM/bpt.py
...
def blockwise_ffn_moe(remat_ffn, inputs, chunk_size=2048, enable_dropout=False):
    # remat_ffn: a rematerialized ffn with policy jax.checkpoint_policies.nothing_saveable()
    # inputs: (batch, seq_len, dim)
    # chunk_size: the chunk size to split the sequence
    inputs = rearrange(inputs, '(c n) d -> c n d', c=chunk_size)
    def scan_ffn(remat_ffn, carry, hidden_states):
        outputs = remat_ffn(hidden_states, enable_dropout=enable_dropout)
        return carry, outputs
    scan_axis = inputs.ndim - 2
    _, res = nn.scan(
        scan_ffn,
